---
title: "Random forest supervivencia meningitis ED_0"
...
#RF_door1, RF_door2 y RF_ED0 recogen solo doormat1 2 o 3 
...
```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
set.seed(123)
mod2<-rfsrc(Surv(tiempo,Muerte_atribuible )~ED_0+Cambio+ Glasgow+ Charlson_Index+
              cambiodat+final+PCR2+UCI+diagetiol2+ulti2+recepcion+Virica+final, 
            data=RF_ED0,ntree = 500, block.size = 1,nodesize = 1,mtry=8,importance = T,na.action = c( "na.impute"))
mod2
```    

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
plot.variable(mod2, partial=T,plots.per.page = 2, xvar.names = c("ED_0","Cambio", "Glasgow", "Charlson_Index","cambiodat","final","PCR2","UCI","diagetiol2","ulti2",
...
b_datos<-dplyr::select(RF_ED0,tiempo,Muerte_atribuible,ED_0,Cambio, Glasgow, Charlson_Index,cambiodat,final,PCR2,UCI,diagetiol2,ulti2,recepcion,Virica,final)
...
Incluye las variables ED_0, Cambio, Glasgow, PCR2, diagetiol2, ulti2, Charlson_Index

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
mf1<-rfsrc(Surv(tiempo,Muerte_atribuible )~ED_0 + Cambio + Glasgow + PCR2+ diagetiol2+
           ulti2+Charlson_Index,data=RF_ED0,ntree = 1000,nodesize = 3)
mf1
```    
