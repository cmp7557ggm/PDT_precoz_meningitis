---
title: "Random forest supervivencia meningitis DOORMAT3"
...
#RF_door1, RF_door2 y Rf_door3 recogen solo doormat1 2 o 3 
...
```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
set.seed(123)
mod2<-rfsrc(Surv(tiempo,Muerte_atribuible )~DOORMAT3+Cambio+ Glasgow+ Charlson_Index+
              cambiodat+final+PCR2+UCI+diagetiol2+ulti2+recepcion+Virica+final, 
            data=RF_door3,ntree = 500, block.size = 1,nodesize = 1,mtry=8,importance = T,na.action = c( "na.impute"))
mod2
```    

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
plot.variable(mod2, partial=T,plots.per.page = 2, xvar.names = c("DOORMAT3","Cambio", "Glasgow", "Charlson_Index","cambiodat","final","PCR2","UCI","diagetiol2","ulti2",
...
b_datos<-dplyr::select(RF_door3,tiempo,Muerte_atribuible,DOORMAT3,Cambio, Glasgow, Charlson_Index,cambiodat,final,PCR2,UCI,diagetiol2,ulti2,recepcion,Virica,final)
...
Incluye las variables DOORMAT3, Cambio, Glasgow, PCR2, diagetiol2, ulti2, Charlson_Index

```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
mf1<-rfsrc(Surv(tiempo,Muerte_atribuible )~DOORMAT3 + Cambio + Glasgow + PCR2+ diagetiol2+
           ulti2+Charlson_Index,data=RF_door3,ntree = 1000,nodesize = 3)
mf1
```    
...
xvar <- c("Glasgow","diagetiol2","DOORMAT3")
...
plot.variable(mf1, partial=T,plots.per.page = 1, xvar.names = c("DOORMAT3"),
              notch=FALSE,surv.type = "surv",time = 12,
              main="DOORMAT3 y supervivencia a 12 días",
              xlab = "Puntuación DOORMAT")  

plot(gg_rfsrc(mf1,by="DOORMAT3"),show.legend = F)+
  labs(x="Tiempo en días")+
  coord_cartesian(xlim = c(0,30))+
  geom_vline(aes(xintercept=9),col="gray")+
  scale_color_discrete("DOORMAT3")+ #, labels = c("No PCR", "PCR"))+
  labs(title = "Curvas supervivencia a 30 días. Variable DOORMAT3")+
...
```{r, echo=FALSE,warning=FALSE,comment="",message=FALSE}
ggplot(pbc_ggpart[["DOORMAT3"]], aes(y=yhat, x=DOORMAT3, col=group))+
geom_boxplot(notch = FALSE,
                 outlier.shape = NA) + # panel=TRUE,
  labs(x = "Puntuación DOORMAT", y = "Survival (%)", color="Time", shape="Time") +
  theme(legend.position = c(0.1, 0.2)) +
  labs(title='Supervivencia según  DOORMAT3', 
       subtitle='A los 7 y a los 30 días')
```
