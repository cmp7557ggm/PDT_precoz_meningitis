---
title: "Regresiones  para cambiodat"
...
mening<-dplyr::select(mening,-c(Fecha_h_meningitis,Fecha_h_LCR,Fecha_h_empirico,Fecha_h_recepcion,
                                Fecha_h_rapida, Fecha_h_resultado_final, Fecha_h_alta, Fecha_cambio, 
                                fecha_diag_etiol,NHC , Procedencia, Desde_observación, 
                                Desde_diag_LCR,Adquisición, TABP_tipo,Tipo_empirico , Desde_diag_empirico, 
                                Desde_diag_recepcion, Celulas, Hematies, PMN, Mono, Glucosa, Proteína, 
                                Tipo_prueba_rapida,Resultado_antigeno, Resultado_Gram, PCR, Tipo_dirigido1, 
                                Tipo_dirigido2, Tipo_dirigido3,Desde_meningitis_dirigido1,
                                Desde_diag_dirigido2,
                                Desde_diag_final, Fecha_h_ingreso, 
                                Fecha_h_dirigido1,Fecha_h_provi, Fecha_h_dirigido2, Fecha_h_dirigido3, 
                                Fecha_altaUCI, 
                                diagetiol,diagetiol2,lcr,emp,rapida,recepcion,final,T_dirigido3_mixto,
                                Diagnostico_sindromico))

mening = mening [ , c(58,1:57,59)]
...
mening_n<-dplyr::select(mening,c(cambiodat,Año,Espectro_DOORMAT0,Espectro_DOORMAT1,Espectro_DOORMAT2,Espectro_DOORMAT3,Resultado_cultivo, Agente_etiologico_detectado))
...
Ajustamos todos los modelos posibles mediante combinaciones con 2, 3, 4 y 5 variables. Se obtienen los r cuadrado, AIC y errores standard de los residuos y se tabulan los resultados.  

En la siguiente tabla se muestran los cinco modelos con mejor bondad de ajuste:  

```{r , echo=FALSE, warning=FALSE,comment=F}
########################3
bdred<-dplyr::select(mening,cambiodat,Año,Virica,Cancer, Diabetes,Systemic_glucocorticoid)
...
Un modelo final factible puede ser:

```{r , echo=FALSE, warning=FALSE,comment=F}
MX3<-lm(cambiodat ~Año+Virica,data=mening)
smx<-summary(MX3)
...
El tiempo hasta el primer cambio de tratamiento se asocia con el año, de manera que tomando como referencia el año 2021, los tiempos son significativamente más largos en los años 2017 y 2018. Esto acontece independientemente de que la meningitis sea de origen bacteriano o vírico.  
