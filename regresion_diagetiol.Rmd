---
title: "Regresiones para tiempo hasta el diagnóstico etiológico"
...
## Variable dependiente tiempo desde la toma de LCR hasta el diagnóstico etiológico 
...
mening<-dplyr::select(mening,-c(Fecha_h_meningitis,Fecha_h_LCR,Fecha_h_empirico,Fecha_h_recepcion,                               Fecha_h_rapida, Fecha_h_resultado_final, Fecha_h_alta, Fecha_cambio, fecha_diag_etiol,NHC , Procedencia, Desde_observación, Desde_diag_LCR,Adquisición, TABP_tipo,Tipo_empirico , Desde_diag_empirico, Desde_diag_recepcion, Celulas, Hematies, PMN, Mono, Glucosa, Proteína, Tipo_prueba_rapida,Resultado_antigeno, Resultado_Gram, PCR, Tipo_dirigido1, Tipo_dirigido2, Tipo_dirigido3, Actividad1,Desde_meningitis_dirigido1,ACTIVO_dirigido2,Desde_diag_dirigido2,
                            Desde_diag_final, ACTIVO_dirigido3,`Mortalidad(SINO)`, Fecha_h_ingreso, Fecha_h_dirigido1,Fecha_h_provi, Fecha_h_dirigido2, Fecha_h_dirigido3, Fecha_altaUCI, Perfil_DOORMAT,
                                final,diagetiol2,lcr,emp,rapida,recepcion,cambiodat,T_dirigido3_mixto,
...
mening_n<-dplyr::select(mening,c(diagetiol,Año,Espectro_DOORMAT0,Espectro_DOORMAT1,Espectro_DOORMAT2,Espectro_DOORMAT3,Resultado_cultivo, Agente_etiologico_detectado))
...
Con las variables categóricas con más de dos categorias se ajustan tambien las correspondientes regresiones simples. Se muestra la regresión con la variable año, que es la unica con alguna categoría significativa:  
...
ml20<- glm(diagetiol ~ Año,data=mening_n)              
...
En un principio disponemos de 5 variables. La variable ulti2 resume el año en dos categorias (antes de 2020 o despues de 2019), que es la que utilizaremos en este caso, dado que la variable cruda año nos aporta poca información.  
...
```{r , echo=FALSE, warning=FALSE,comment=F}
########################3
bdred<-dplyr::select(mening,diagetiol,Año,ulti2,Chronic_Kidney_disease,Systemic_glucocorticoid)
...
ccc<-combinar(bdred,c(2:5),1)
d<-Mconf(bdred,ccc)
#d

ccc<-combinar(bdred,c(2:5),2)
dd<-Mconf(bdred,ccc)
#dd

ccc<-combinar(bdred,c(2:5),3)
ddd<-Mconf(bdred,ccc)
#ddd 
...
De los  modelos incluidos en el top 5, finalmente optamos por el modelo 4.2 que incluye tres variables: el año dicotomizado (ulti2), los antecedentes de enfermedad renal crónica y el uso de glucocorticoides sistémicos. 

El modelo final queda como sigue:  

```{r , echo=FALSE, warning=FALSE,comment=F}
MX3<-lm(diagetiol ~ulti2+Chronic_Kidney_disease+Systemic_glucocorticoid,data=mening)
smx<-summary(MX3)
...
Considerando el tiempo que transcurre desde la toma de LCR hasta que se consigue el diagnóstico etiologico, podemos afirmar que éste es significativamente menor en el periodo 2020-2021  que en los años anteriores. Concretamnen, el tiempo se reduce de media en 16.6 horas. 

Presentar antecedentes de enfermedad renal crónica  y el uso de glucocorticoides sitémicos  se asocian con un tiempo hasta el diagnóstico etiológico mas largo. En estos pacientes, si el resto de variables permanencen constantes, identificar el microorganismo tarda, de media, 28 horas más.
